@Library('Shared') _
pipeline {
    agent any
    
    environment{
        SONAR_HOME = tool "sonar-server"
        OWASP_HOME = tool "OWASP"
    }
    
    parameters {
        string(name: 'FRONTEND_DOCKER_TAG', defaultValue: '', description: 'Setting docker image for latest push')
        string(name: 'BACKEND_DOCKER_TAG', defaultValue: '', description: 'Setting docker image for latest push')
    }
    
    stages {
        stage("Validate Parameters") {
            steps {
                script {
                    if (params.FRONTEND_DOCKER_TAG == '' || params.BACKEND_DOCKER_TAG == '') {
                        error("FRONTEND_DOCKER_TAG and BACKEND_DOCKER_TAG must be provided.")
                    }
                }
            }
        }
        stage("Workspace cleanup"){
            steps{
                script{
                    cleanWs()
                }
            }
        }
        
        stage('Git: Code Checkout') {
            steps {
                script{
                    code_checkout("https://github.com/nuruzzaman24x/mega-project-01","main")
                }
            }
        }
        
        stage("Trivy: Filesystem scan"){
            steps{
                script{
                    trivy_scan()
                }
            }
        }

        stage("OWASP: Dependency check"){
            steps{
                script{
                    owasp_dependency()
                }
            }
        }
        
        // stage("SonarQube: Code Analysis"){
        //     steps{
        //         script{
        //             sonarqube_analysis("sonar-server","mega","mega")
        //         }
        //     }
        // }
        
        
        
        
        stage("SonarQube: Backend Analysis"){
    steps{
        script{
            dir("backend"){
                sonarqube_analysis("sonar-server","mega-backend","mega-backend")
            }
        }
    }
}

stage("SonarQube: Frontend Analysis"){
    steps{
        script{
            dir("frontend"){
                sonarqube_analysis("sonar-server","mega-frontend","mega-frontend")
            }
        }
    }
}
        
        
        
        
        
        stage("SonarQube: Code Quality Gates"){
            steps{
                script{
                    sonarqube_code_quality()
                }
            }
        }
        
        stage('Exporting environment variables') {
            parallel{
                stage("Backend env setup"){
                    steps {
                        script{
                            dir("Automations"){
                                sh "bash updatebackendnew.sh"
                            }
                        }
                    }
                }
                
                stage("Frontend env setup"){
                    steps {
                        script{
                            dir("Automations"){
                                sh "bash updatefrontendnew.sh"
                            }
                        }
                    }
                }
            }
        }
        
        stage("Docker: Build Images"){
            steps{
                script{
                        dir('backend'){
                            docker_build("mega-backend-beta","${params.BACKEND_DOCKER_TAG}","nuruzzaman24x")
                        }
                    
                        dir('frontend'){
                            docker_build("mega-frontend-beta","${params.FRONTEND_DOCKER_TAG}","nuruzzaman24x")
                        }
                }
            }
        }
        
        stage("Docker: Push to DockerHub"){
            steps{
                script{
                    docker_push("mega-backend-beta","${params.BACKEND_DOCKER_TAG}","nuruzzaman24x") 
                    docker_push("mega-frontend-beta","${params.FRONTEND_DOCKER_TAG}","nuruzzaman24x")
                }
            }
        }
    }
    post{
        success{
            archiveArtifacts artifacts: '*.xml', followSymlinks: false
            build job: "mega-CD", parameters: [
                string(name: 'FRONTEND_DOCKER_TAG', value: "${params.FRONTEND_DOCKER_TAG}"),
                string(name: 'BACKEND_DOCKER_TAG', value: "${params.BACKEND_DOCKER_TAG}")
            ],
            propagate: false
        }
    }
}




// @Library('Shared') _
// pipeline {
//     agent {label 'Node'} # agent any; if you are running Jenkins on one (master) machine
    
//     environment{
//         SONAR_HOME = tool "sonar-server"
//     }
    
//     parameters {
//         string(name: 'FRONTEND_DOCKER_TAG', defaultValue: '', description: 'Setting docker image for latest push')
//         string(name: 'BACKEND_DOCKER_TAG', defaultValue: '', description: 'Setting docker image for latest push')
//     }
    
//     stages {
//         stage("Validate Parameters") {
//             steps {
//                 script {
//                     if (params.FRONTEND_DOCKER_TAG == '' || params.BACKEND_DOCKER_TAG == '') {
//                         error("FRONTEND_DOCKER_TAG and BACKEND_DOCKER_TAG must be provided.")
//                     }
//                 }
//             }
//         }
//         stage("Workspace cleanup"){
//             steps{
//                 script{
//                     cleanWs()
//                 }
//             }
//         }
        
//         stage('Git: Code Checkout') {
//             steps {
//                 script{
//                     code_checkout("https://github.com/nuruzzaman24x/mega-project-01","main")
//                 }
//             }
//         }
        
//         stage("Trivy: Filesystem scan"){
//             steps{
//                 script{
//                     trivy_scan()
//                 }
//             }
//         }

//         stage("OWASP: Dependency check"){
//             steps{
//                 script{
//                     owasp_dependency()
//                 }
//             }
//         }
        
//         stage("SonarQube: Code Analysis"){
//             steps{
//                 script{
//                     sonarqube_analysis("sonar-server","mega","mega")
//                 }
//             }
//         }
        
//         stage("SonarQube: Code Quality Gates"){
//             steps{
//                 script{
//                     sonarqube_code_quality()
//                 }
//             }
//         }
        
//         stage('Exporting environment variables') {
//             parallel{
//                 stage("Backend env setup"){
//                     steps {
//                         script{
//                             dir("Automations"){
//                                 sh "bash updatebackendnew.sh"
//                             }
//                         }
//                     }
//                 }
                
//                 stage("Frontend env setup"){
//                     steps {
//                         script{
//                             dir("Automations"){
//                                 sh "bash updatefrontendnew.sh"
//                             }
//                         }
//                     }
//                 }
//             }
//         }
        
//         stage("Docker: Build Images"){
//             steps{
//                 script{
//                         dir('backend'){
//                             docker_build("mega-backend-beta","${params.BACKEND_DOCKER_TAG}","bongodev")
//                         }
                    
//                         dir('frontend'){
//                             docker_build("mega-frontend-beta","${params.FRONTEND_DOCKER_TAG}","bongodev")
//                         }
//                 }
//             }
//         }
        
//         stage("Docker: Push to DockerHub"){
//             steps{
//                 script{
//                     docker_push("mega-backend-beta","${params.BACKEND_DOCKER_TAG}","bongodev") 
//                     docker_push("mega-frontend-beta","${params.FRONTEND_DOCKER_TAG}","bongodev")
//                 }
//             }
//         }
//     }
//     post{
//         success{
//             archiveArtifacts artifacts: '*.xml', followSymlinks: false
//             build job: "mega-CD", parameters: [
//                 string(name: 'FRONTEND_DOCKER_TAG', value: "${params.FRONTEND_DOCKER_TAG}"),
//                 string(name: 'BACKEND_DOCKER_TAG', value: "${params.BACKEND_DOCKER_TAG}")
//             ],
//             propagate: false
//         }
//     }
// }
